...
- [ ] Implement `RaftLogStore` against git-pinned `raft-engine`; raft-engine's internal `lz4-sys` compression disabled in config; compression (if any) done above the engine in `mango-raft` with `lz4_flex` per inventory and ADR 0002 §W5
