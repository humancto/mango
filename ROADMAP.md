...
- [ ] Define `Backend` + `RaftLogStore` trait pair per **ADR 0002 §6**: `Backend` has associated `Snapshot`/`Batch` types and `register_bucket` / `snapshot` / `begin_batch` / `commit_batch(force_fsync)` / `commit_group` / `open` / `close` / `size_on_disk` / `defragment`. `RaftLogStore` has `append` / `entries` / `last_index` / `first_index` / `compact` / `install_snapshot` / `save_hard_state` / `hard_state`. All engine-specific types hidden behind associated types; GAT-lending range iterator; async on the write path, sync on the read path.
