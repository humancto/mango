...
- [ ] Implement `Backend` against `redb` with on-disk durability and `fsync` semantics at least as strong as etcd's batch-tx model (commit on N writes or T millis), with the batching parameters tunable; `commit_group` is the primitive for Raft fsync batching (ADR 0002 §6 design point 4)
