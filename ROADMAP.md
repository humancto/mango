...
| Storage engine                                  | `redb` (KV) + `tikv/raft-engine` (Raft log)                          | iroh, Cuprate (redb); TiKV at multi-PB scale (raft-engine)                                                                                                                                | **Pure-Rust COW B-tree** (redb) for the KV; **log-structured segmented append** (raft-engine) for the Raft log. Both behind a `Backend` + `RaftLogStore` trait pair in `crates/mango-storage`. **Unsafe stance**: redb has 37 `unsafe` tokens in src/ (v4.1.0), raft-engine has 49 on master — both tracked in the Phase 0.5 `cargo-geiger` baseline; +10 regression triggers ADR refresh. Mango's workspace stays `unsafe_code = "forbid"`. **raft-engine specifics**: pinned via git+rev SHA (crates.io 0.4.2 is 24 months stale while master is active through 2026-03-10); default `lz4-sys` C FFI compression is disabled — compression (if any) is done above the engine with `lz4_flex` for inventory consistency (see row below). **Runner-up** `heed`+LMDB (retained as escape hatch). Documented alternatives: `rust-rocksdb` for write-heavy LSM workloads where the C++ blast surface is acceptable; **`fjall`** as the pure-Rust LSM alternative (workload-mismatched for the GB-scale read-dominated etcd profile). `sled` ruled out by its own maintainer. Hand-roll ruled out on team shape. **See `.planning/adr/0002-storage-engine.md` for the full decision.** |
...
| Block compression                               | `lz4_flex` (default), `zstd` (high-ratio)                            | TiKV, polars                                                                                                                                                                              | Phase 1 backend block compression (configurable). `lz4_flex` is the pure-Rust default; `zstd` is C-backed but standard. **raft-engine's built-in `lz4-sys` (C FFI) compression is disabled in mango's raft-engine config**; compression above raft-engine uses `lz4_flex` for inventory consistency. See ADR 0002 §Warts W5.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
- [x] Choose the storage engine — decided in **`.planning/adr/0002-storage-engine.md`**: `redb` 4.1.0+ (KV) + `tikv/raft-engine` git-pinned to a master SHA (Raft log), both behind a `Backend` + `RaftLogStore` trait pair. Every claim verified against primary sources (`.planning/adr/0002-storage-engine.verification.md`, 2026-04-24). Runner-up `heed`+LMDB retained as the escape hatch. **Decision criterion: must beat the bbolt comparison oracle in `benches/oracles/bbolt/` (a checked-in Go binary running the workload defined in `benches/workloads/storage.toml` against bbolt at a pinned version) on at least one of (write throughput, read latency at p99, on-disk size for the same dataset, fsync amplification) without losing on the others.**
- [x] `crates/mango-storage` skeleton with `redb` + git-pinned `raft-engine` as dependencies (raft-engine compression disabled; lz4_flex above the engine per ADR 0002 §W5)
- [x] Define `Backend` + `RaftLogStore` trait pair per **ADR 0002 §6**: `Backend` has associated `Snapshot`/`Batch` types and `register_bucket` / `snapshot` / `begin_batch` / `commit_batch(force_fsync)` / `commit_group` / `open` / `close` / `size_on_disk` / `defragment`. `RaftLogStore` has `append` / `entries` / `last_index` / `first_index` / `compact` / `install_snapshot` / `save_hard_state` / `hard_state`. All engine-specific types hidden behind associated types; GAT-lending range iterator; async on the write path, sync on the read path.
- [x] Implement `Backend` against `redb` with on-disk durability and `fsync` semantics at least as strong as etcd's batch-tx model (commit on N writes or T millis), with the batching parameters tunable; `commit_group` is the primitive for Raft fsync batching (ADR 0002 §6 design point 4)
- [x] Implement `RaftLogStore` against git-pinned `raft-engine`; raft-engine's internal `lz4-sys` compression disabled in config; compression (if any) done above the engine in `mango-raft` with `lz4_flex` per inventory and ADR 0002 §W5
- [x] **Differential-test harness vs bbolt** `tests/differential/backend_vs_bbolt.rs`: proptest-generated 10k-operation sequences run against mango's `Backend` and against the `benches/oracles/bbolt/` Go binary via JSON IPC; assert identical visible state after every commit. Blocker for Phase 1 close. **Non-bbolt-quirk divergence that cannot be root-caused to a wrapper bug triggers engine swap** per ADR 0002 §5 Tier-1 trigger #2.
- [ ] **7-day sustained chaos gate** `tests/chaos/backend_7day.rs`: run `benches/workloads/storage.toml` under disk-EIO + SIGKILL injection for 7 continuous wall-clock days on the `benches/runner/HARDWARE.md` sig. No corruption, no data loss, no panic. Required before Phase 1 is marked done. Operationalizes the "no marquee production user" redb risk documented in ADR 0002 §W1.
- [x] **Engine-swap dry-run test** `tests/migration/engine_swap_dryrun.rs`: swap `Backend` trait impl from redb to an in-memory reference impl mid-test and confirm data survives and semantics match. Proves the trait boundary from ADR 0002 §6 is actually swappable; defends the 2-week migration budget in ADR 0002 §Migration.
- [x] **Recovery-time measurement** in the crash-recovery test harness: report wall-clock recovery time at 1 GiB, 4 GiB, 8 GiB of data after unclean shutdown. **Budget: ≤ 30 seconds at 8 GiB** per ADR 0002 §W8; exceeding this triggers engine swap per §5 Tier-1 trigger #4.
- [x] **cargo-geiger baseline pin** for storage deps: `redb` and `raft-engine` per-category totals + canonical `(name, source, version)` pinned in `unsafe-baseline.json#storage_deps` (numbers anchored from the geiger CI run, not the ROADMAP — `docs/unsafe-policy.md#storage-dep-coverage-roadmap823-adr-0002-5-trigger-8` covers schema, bumping, flake-remediation). Per-category +10 over baseline trips CI (`scripts/geiger-check.sh`, exit 6) and requires ADR 0002 §5 trigger #8 refresh before the bump merges; source/version drift trips exit 7 (re-pin via `MANGO_GEIGER_REPIN=1`). The baseline is dormant when `storage_deps_required: false` (bootstrap window only); flipping to `true` is the activation event.
- [x] Property tests: random put/get/delete/range sequences match an in-memory `BTreeMap` oracle (proptest, 10k+ cases)
- [x] Crash-recovery test: kill mid-write via a panic, reopen, assert no torn state and no committed data lost
- [x] Crash-recovery test under simulated fsync failure (return `EIO`) — backend either commits cleanly or reports failure; no silent data loss
...
- [x] Compaction: physically removes old revisions; `Range` against a compacted revision returns `ErrCompacted`
...
- [x] Property tests: random ops + random snapshot reads match a model implementation (proptest, 10k+ cases, shrinking on)
- [ ] Restore-from-disk test: persist via backend, drop the in-memory index, reopen, all reads consistent
- [x] **`cargo fuzz` target for the on-disk key encoding** (`encode_key`/`decode_key` round-trip, plus malformed-input → no panic). Per the Reviewer's contract: parser fuzz lives in the phase that introduces the parser, not deferred to Phase 15. CI plumbing for the fuzz job lives in Phase 15.
...
- [ ] WAL: append every entry before applying; replay on startup; **bounded WAL space** with retention by size + time, oldest segment recycled or deleted post-snapshot. Documented behavior when WAL disk fills (refuse new proposals, raise `WAL_FULL` alarm). **Implementation**: WAL is the `RaftLogStore` trait from ADR 0002, backed by git-pinned `tikv/raft-engine` (append-fsync-batched per raft-engine's `LogBatch`). Compression disabled at the engine level per ADR 0002 §W5.
